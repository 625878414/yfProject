    ...
    <div class="header-tag-list" :style="'left:'+offsetLeft+'px;'">
      ...
      <div  group="tag"  v-for="(tag,index) in menuCache" :key="'tag-'+index" class="tag-draggable"  :id="'les-tag-'+index">
        ...
        <a-tag  @click="changeTag(tag.name)" :color="tag.name==currentRoute?'#1890ff':''"
               >
          <i class="tag-circle" :class="tag.name==currentRoute?'active':''" />{{tag.title}}<a-icon type="close" :class="homeMenuId!=tag.name?'tag-close':'none'" @click.stop="closeTag(tag.name,index)"/>
        </a-tag>
      </div>
    </div>
    <a-icon type="caret-right" class="header-tag-right" @click="rightMove" />
    <div class="header-icon-close" style="width:50px">
