  ...
  <div class="Form">
    ...
    >
      ...
      <div v-show="formChild.createIndex" class="selectIndex">
        ...
        <a-form-model-item label="索引字段" style="margin-bottom:0px" >
          <span>
            索引构成:
            {{ idShow.join("_") }}
          </span>
    ...
    <!-- <a-button @click="see">查看标签</a-button> -->
  </div>
</template>
...
export default {
  ...
  data() {
    const checkRepeat = (rule, value, callback) => {
      this.$http("/zuul/lmanage/dataEntity/name/isExist?name=" + this.formChild.name).then(res => {
  ...
  methods: {
    ...
    getLabelist() {
      this.$http.get("/zuul/lmanage/label/dict").then(res => {
    ...
    handelChange(value) {
      console.log(value);
      console.log(this.formChild.classificationNames);
      this.formChild.classificationNames.forEach((item,index)=>{
        if(item.length>10){
          this.formChild.classificationNames.splice(index,1)
        }
      })
    },
    handelSelect(value) {
      if(value.length>10){
        this.$message.error("标签长度应该小于10个字符",1)
        let index=this.formChild.classificationNames.findIndex(item=>{
          return item===value
        })
        this.formChild.classificationNames.splice(index,1)
        // console.log(this.formChild.classificationNames);
    ...
    aliasesChange() {
      if (!this.isEdit && !this.inputSelf) {
        this.formChild.name = this.inputPre;
      }
      if (!this.isEdit && !this.inputSelf) {
        this.formChild.name = this.inputPre + pinyin.getCamelChars(this.formChild.aliases);
        console.log(this.formChild.name);
        console.log(this.inputPre);
        this.$refs.formModel.validateField('name');
      }
      // console.log(123);
    },
    nameChange(value) {
