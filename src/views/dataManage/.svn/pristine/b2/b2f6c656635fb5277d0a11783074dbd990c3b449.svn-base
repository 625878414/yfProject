  ...
  <div class="processList">
      ...
      >
        <template slot="status" slot-scope="text, record">
          <a-tag
            ...
            v-if="text > 0"
            class="tag"
            @mouseenter="showStatus(record)"
            @mouseleave="leave"
            >运行</a-tag
          >
          <a-tag color="red" v-else class="tag" @mouseenter="showStatus(record)" @mouseleave="leave"
            >停止</a-tag
          >
        </template>
        ...
        <div class="form">
          ...
          <a-form-model :model="formData" layout="vertical" ref="formModel" :rules="ruleValidate">
            <a-form-model-item label="流程名称" prop="name">
              <a-input v-model="formData.name" placeholder="请输入流程名称" :maxLength="20" />
            </a-form-model-item>
    ...
    >
      <a-table
        ...
        :scroll="{ x: `${previewCol.length * 300}px` , y:`${bodyHeight-150}px`}"
      >
      </a-table>
    </a-drawer>
    <!-- 状态模态框 -->
      ...
      <a-spin size="large" :spinning="spinDetails">
        <a-descriptions title="元数据详情" bordered :column="1" size="small">
          <a-descriptions-item label="名称">
            {{ dataEntityDetails.name }}
          </a-descriptions-item>
          <a-descriptions-item label="中文名称">
            {{ dataEntityDetails.aliases }}
          </a-descriptions-item>
          <a-descriptions-item label="存储类型">
            {{ dataEntityDetails.typeName }}
          </a-descriptions-item>
          <a-descriptions-item label="来源类型">
            {{ dataEntityDetails.originTypeName }}
          </a-descriptions-item>
          <a-descriptions-item label="标签">
            {{ dataEntityDetails.classificationNames }}
          </a-descriptions-item>
          <a-descriptions-item label="描述">
            {{ dataEntityDetails.description }}
          </a-descriptions-item>
          <a-descriptions-item label="来源表名">
            {{ dataEntityDetails.dataSourceFile }}
          </a-descriptions-item>
        ...
        <div style="margin-top: 20px">
          <a-descriptions title="所属数据源详情" bordered :column="1" size="small">
            <a-descriptions-item label="所属数据源名称">
              {{ dataEntityDetails.dataSource.name }}
            </a-descriptions-item>
            <a-descriptions-item label="所属数据源类型">
              {{
                dataEntityDetails.dataSource.sourceType == "0"
                  ? "BigData"
                  : dataEntityDetails.dataSource.sourceType == "1"
                  ? "JDBC"
                  : "Kafka"
              }}
            </a-descriptions-item>
            <a-descriptions-item label="所属数据源URL">
              {{ dataEntityDetails.dataSource.url }}
            </a-descriptions-item>
          </a-descriptions>
        </div>
        <!-- <p class="columns-details-title">列信息详情</p> -->
        <div class="columns-details-title">
          <h3 style="margin: 20px 0px">列信息详情</h3>
          <vue-scroll :style="'height:' + (bodyHeight - 700) + 'px;'">
            <a-table
              size="small"
  ...
  data() {
    return {
      ...
      tableData: [
        // {
        //   name:"过车流程",
        //   scheduleMethod:"Timer driven",
        //   dataEntityName:"ysj_gc",
        //   runningCount:0,
  ...
  methods: {
            ...
            res.data.data.columns.forEach(item => {
              if (item.columnName == key.toUpperCase()) {
                this.previewCol.push({
                  title:
                    item.comment == "" || item.comment == null ? key : key + `(${item.comment})`,
                  dataIndex: key,
                  align: "left"
                });
              }
    ...
    addClose() {
      this.$confirm({
        title: "提示",
        content: h => (
            <h3 style="color:red">流程还未保存,是否关闭</h3>
        ),
        okText: "确认",
        okType: "danger",
        cancelText: "取消",
    ...
    showDetails(row) {
      ...
      this.$http({
        ...
        method: "get",
        url: "/zuul/lmanage/dataEntity/detail"
      }).then(res => {
        if (res.data.success) {
          console.log(res.data.data);
          this.dataEntityDetails = res.data.data;
          if (this.dataEntityDetails.esColumns != "") {
            let esColumns = JSON.parse(this.dataEntityDetails.esColumns);
            this.elements = esColumns[esColumns.length - 1].elements;
          }
          this.columnDetails = this.dataEntityDetails.columns;
        } else {
          this.$message.error("获取详情失败", 2);
        }
