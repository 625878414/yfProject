<template>
  <div class="catalog">
    ...
    <div style="margin:0 auto;width:1400px;height:100%;margin-bottom:100px">
      <a-table
        :columns="columns"
        :data-source="data"
        rowKey="id"
    ...
    >
      <a-form-model layout="vertical" :rules="ruleValidate" ref="formModel" :model="formData">
        <a-form-model-item label="目录" prop="name">
          <a-input
            placeholder="请输入中文目录"
        ...
        <a-form-model-item label="责任人">
          <a-input v-model="formData.master" placeholder="请输入责任人" :maxLength="10"></a-input>
        </a-form-model-item>
  ...
  methods: {
    add(row) {
      ...
      if (row.parentIds === null) {
        ...
        if (row.parentIds.split(",").length >= 2) {
          this.$message.error("目录最多三级", 1);
          return;
        } else {
          this.drawerTitle = "新增下级目录";
          this.formData = {
    ...
    save() {
      this.$refs.formModel.validate(valid => {
        if (valid) {
          // let parentObj=this.getParentsById(this.data,this.formData.parentId)
          // console.log(parentObj[0]);
          // if(parentObj[0].children && parentObj.length>0){
          //   var isRepeat=parentObj[0].children.some(item=>{
          //     return item.name===this.formData.name
          //   })
          // }
          // if(isRepeat){
          //   this.$message.error("目录名称重复,请检查",1)
          //   return
          // }
          // let flag = this.catalogId.some(item => {
          //   return item === this.formData.id;
          // });
          // console.log(flag);
          // if (flag) {
          //   this.$message.error("目录修改错误,不可修改为自身或者子级", 1);
          //   return;
          // }
    ...
    edit(row) {
      ...
      this.$http.get("/zuul/lmanage/asset/catalog/" + row.id).then(res => {
        ...
        // this.formData.catalogId=res.data.data.parentIdsList
        this.catalogId = this.formData.parentIdsList;
