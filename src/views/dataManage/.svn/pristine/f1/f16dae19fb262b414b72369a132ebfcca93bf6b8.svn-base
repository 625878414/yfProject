              ...
              <div class="les-table-toolbar">
                ...
                <a-button-group>
                  <a-button
                    ...
                    icon="tool"
                    @click="oneClickBind"
                    :loading="oneBtnLoading"
                    :disabled="matchTable.length===0"
                  >
                    一键绑定
                  </a-button>
                </a-button-group>
              ...
              <vue-scroll style="height: 500px">
                ...
                >
                  <!-- <template slot="action" slot-scope="record">
                    <a-button
                      type="danger"
                      :loading="record.cancelLoading"
                      size="small"
                      @click="cancelBinding(record)"
                      v-if="record.bindFieldId"
                      >取消绑定</a-button
                    >
                    <a-button
                      type="primary"
                      size="small"
                      :loading="record.bindLoading"
                      @click="bind(record)"
                      v-else
                      >强制绑定</a-button
                    >
...
export default {
  ...
  data() {
    return {
      ...
      hitColumn: [
        ...
        {
          dataIndex: "bindFieldId",
          title: "绑定情况",
          align: "center",
          // width: "auto",
          customRender: (text) => (text ? "已绑定" : "未绑定"),
        },
        {
          dataIndex: "type",
          title: "数据类型",
  ...
  methods: {
    ...
    bind(record) {
      this.$set(record, "bindLoading", true);
      this.$http({
        method: "post",
        data: {
        ...
        url: "/zuul/lmanage/dataStandard/field/forceBind",
      }).then((res) => {
        if (res.data.success) {
          this.$notification.success({
            message: "绑定成功",
          ...
          this.tabKey === "1" ? this.renderMatchTable() : this.renderHitTable();
        } else {
          this.$notification.error({
            message: "绑定失败",
            duration: 2,
        ...
        record.bindLoading = false;
      });
    },
    cancelBinding(record) {
      console.log(record);
      this.$set(record, "cancelLoading", true);
      this.$http
        .delete("/zuul/lmanage/dataStandard/field/unBind/" + record.fieldId)
        .then((res) => {
          if (res.data.success) {
            this.$notification.success({
              message: "取消绑定成功",
          ...
          record.cancelLoading = false;
        });
    },
    oneClickBind() {
      this.oneBtnLoading = true;
      this.$http({
        method: "post",
        url: "/zuul/lmanage/dataStandard/field/oneClickBind/"+this.searchMatch.standardId,
      }).then((res) => {
        if (res.data.success) {
          this.$notification.success({
            message: "一键绑定成功",
