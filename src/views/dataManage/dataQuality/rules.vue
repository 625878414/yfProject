  ...
  <div class="dataQuality-rules">
            ...
            <div class="rule-form" v-for="(rule, index) in formData.rules" :key="index">
              ...
              <a-row :gutter="20">
                <a-col :span="7">
                  ...
                  >
                    <a-select style="width:160px" v-model="rule.columnName" placeholder="请选择">
                      <a-select-option
                ...
                <a-col :span="8" v-if="rule.type != 4">
                  ...
                  >
                    ...
                    <a-select
                      v-else-if="rule.type == 1"
                      style="width:160px"
                      v-model="rule.functionId"
                      placeholder="请选择"
                    >
                      <a-select-option
                        v-for="(item, index) in funcList1"
                        :key="index"
                        :value="item.id"
                      >
                        {{ item.functionName }}
              ...
              <a-row>
                <a-col :span="8">
                  <a-form-model-item
                    ...
                    class="checkCerifyType"
                  >
                    <a-select style="width:160px" v-model="rule.verifyType" placeholder="请选择">
                      <a-select-option
              ...
              <a-row>
                ...
                <a-col :span="2">
                  ...
                  >
                    <a-input
                      ...
                      v-model.number="rule.valueRange.postValue"
                      :maxLength="5"
                    ></a-input>
                  </a-form-model-item>
                </a-col>
                ...
                <a-col :span="1">
                  <a-button
                    @click="statusChangeAdd(rule, index)"
                ...
                <a-col :span="4">
                  ...
                  >
                    <a-select
                      ...
                      placeholder="请选择运算符"
                      :disabled="rule.valueRange.relationType === '0'"
                    >
                      <a-select-option value="="> = </a-select-option>
                      <a-select-option value="<"> &lt; </a-select-option>
                      <a-select-option value=">"> &gt; </a-select-option>
                ...
                <a-col :span="4">
                  ...
                  >
                    <a-input
                      ...
                      v-model.number="rule.valueRange.preValue"
                      :disabled="rule.valueRange.relationType === '0'"
                      :maxLength="5"
                    ></a-input>
                  </a-form-model-item>
      ...
      <div class="steps-content" v-else>
        ...
        >
          <a-form-model-item label="调度周期" prop="periodType">
            <a-select
              placeholder="请选择调度周期"
              v-model="formData.scheduleConf.periodType"
          ...
          <a-form-model-item label="间隔时间" placeholder="请输入间隔" prop="timeInterval">
            <a-input
          ...
          <a-form-model-item label="样本量上限" prop="sampleUpperLimit">
            <a-input-number
              v-model="formData.scheduleConf.sampleUpperLimit"
              :max="9999"
              :min="1"
              :precision="0"
            ></a-input-number>
      ...
      <div v-if="editVisible" class="edit">
        <a-tabs v-model="tabKey">
          <a-tab-pane key="1" tab="规则管理">
            <vue-scroll :style="'height:' + (bodyHeight - 180) + 'px;'">
              ...
              <a-descriptions style="margin:20px 0px" bordered size="small" :column="2">
                <a-descriptions-item label="执行周期">
                  {{ descriptions.scheduleConf.periodType === "0" ? "分" : "时" }}
                </a-descriptions-item>
                ...
                <a-descriptions-item label="间隔时间">
                    ...
                    <a-col :span="4">
                      ...
                      >
                        <a-select
                          ...
                          v-model="rule.valueRange.preOperator"
                          placeholder="请选择运算符"
                          :disabled="rule.notEdit || rule.valueRange.relationType === '0'"
                        >
                          <a-select-option value="="> = </a-select-option>
                          <a-select-option value="<"> &lt; </a-select-option>
                          <a-select-option value=">"> &gt; </a-select-option>
            ...
            <div class="drawer">
              <a-divider style="background:#000"></a-divider>
              <a-button type="danger" style="margin-right:10px" @click="drawerClose">关闭</a-button>
              <a-button type="primary" @click="saveDrawer" :loading="editSaveLoading"
                >保存</a-button
              >
            </div>
          </a-tab-pane>
    ...
    >
      ...
      >
        ...
        <a-form-model-item label="间隔时间" prop="timeInterval">
          <a-input
            v-model="formData.scheduleConf.timeInterval"
            placeholder="请输入间隔时间"
            :maxLength="5"
...
function checkMinute(rule, value, callback) {
  if (value > 0 && value <= 60) {
    callback();
  } else {
    callback(new Error("间隔分钟为1-60"));
  }
}
function checkHour(rule, value, callback) {
  if (value > 0 && value <= 24) {
    callback();
  } else {
    callback(new Error("间隔小时为1-24"));
  }
}
function checkTime(rule, value, callback) {
  let selectStartTime = new Date(value[0]).getTime();
  let selectEndTime = new Date(value[1]).getTime();
  let currentTime = new Date().getTime();
  if (selectStartTime - currentTime < 0 || selectEndTime - currentTime < 0) {
...
export default {
  ...
  methods: {
    ...
    next() {
      ...
      if (this.current === 0) {
        ...
        if (this.formData.rules.length === 0) {
          this.$message.error("请至少添加一项规则", 2);
        } else {
          this.$nextTick(() => {
      ...
      // } else if (this.current === 1) {
      //   if (this.formData.rules.length === 0) {
      //     this.$message.error("请至少添加一条规则", 2);
      //   } else {
      //     this.$nextTick(() => {
      //       this.$refs.ruleValid.validate(valid => {
      //         if (valid) {
    ...
    selectRule(e) {
      ...
      this.getFuncByType(e.key + "");
    },
    //删除新增时的增加的规则 (无需调用接口)
    delRule(index) {
      console.log(index);
      this.$refs.ruleValid.clearValidate([
        `rules.${index}.valueRange.preOperator`,
        `rules.${index}.valueRange.preValue`,
        `rules.${index}.valueRange.postOperator`,
        `rules.${index}.valueRange.postValue`,
        `rules.${index}.verifyType`,
        `rules.${index}.columnName`,
        `rules.${index}.functionId`
      ]);
      ...
      this.$refs.dispatch.validate(valid => {
        if (valid) {
          ...
          this.formData.rules.forEach(item => {
            if (item.valueRange.preOperator === undefined) {
              item.valueRange.preOperator = "";
            }
    ...
    editModalClose() {
      ...
      this.$refs.dispatch.resetFields();
    },
    //点击修改抽屉中的编辑调度按钮
    editDispatch() {
      this.ruleValidate.timeInterval[2] =
        this.formData.scheduleConf.periodType === "0"
          ? { validator: checkMinute, trigger: "change" }
          : this.formData.scheduleConf.periodType === "1"
          ? { validator: checkHour, trigger: "change" }
          : {};
    ...
    saveRule(rule, index) {
      ...
      this.$refs.ruleValid[index].validate(valid => {
        if (valid) {
          ...
          if (rule.valueRange.preOperator === undefined) {
            rule.valueRange.preOperator = "";
          }
          if (rule.valueRange.postOperator === undefined) {
            rule.valueRange.postOperator = "";
          }
          this.$http({
            method: "post",
            data: {
    ...
    statusChangeAdd(rule, index) {
      ...
      if (rule.valueRange.relationType === "0") {
        ...
        rule.valueRange.preValue = "";
        rule.valueRange.preOperator = undefined;
      }
    },
    ...
    statusChange(rule, index) {
      rule.valueRange.relationType =
        rule.valueRange.relationType == "0" ? "1" : rule.valueRange.relationType == "1" ? "2" : "0";
      if (rule.valueRange.relationType === "0") {
        this.$refs.ruleValid[index].clearValidate([
          `rules.${index}.valueRange.preOperator`,
          `rules.${index}.valueRange.preValue`
        ]);
        rule.valueRange.preValue = "";
    ...
    del(record) {
      this.$confirm({
        title: "信息提示",
        content: "删除规则会同步清除校验任务!",
        okType: "danger",
        onOk: () => {
          this.$set(record, "deloading", true);
          this.$http.delete("/zuul/lmanage/dataQuality/monitor/delete/" + record.id).then(res => {
            if (res.data.success) {
              this.$notification.success({
                message: "删除成功",
                duration: 2
    ...
    periodTypeChange(value) {
      this.formData.scheduleConf.timeInterval = "";
      this.$refs.dispatch.clearValidate("timeInterval");
      if (value === "0") {
        this.ruleValidate.timeInterval[2] = { validator: checkMinute, trigger: "change" };
      } else if (value === "1") {
        this.ruleValidate.timeInterval[2] = { validator: checkHour, trigger: "change" };
      }
    }
  },
  created() {},
...
/deep/ .myCheck1 .ant-form-explain {
  width: 100px;
  position: relative;
  right: 2rem;
}
/deep/ .myCheck3 .ant-form-explain {
  width: 100px;
  position: relative;
  right: 2.5rem;
}
/deep/ .myCheck4 .ant-form-explain {
  width: 100px;
  position: relative;
  right: 5.7143rem;
}
/deep/.checkCerifyType .ant-form-explain {
  position: relative;
  left: 5.9286rem;
}
/deep/.checkRange .ant-form-explain {
  position: relative;
  left: 5.9286rem;
}
/deep/.checkRange1 .ant-form-explain {
  position: relative;
  left: 6.4286rem;
}
/deep/.checkRange2 .ant-form-explain {
  position: relative;
  right: 4.2857rem;
}
/deep/.checkRange3 .ant-form-explain {
  position: relative;
  right: 2.8571rem;
}
/deep/.checkRange4 .ant-form-explain {
  position: relative;
  right: 3.5714rem;
}
.dataQuality-rules{
