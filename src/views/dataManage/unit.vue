  ...
  <div class="unit-index">
    ...
    <vue-scroll :style="'height:' + (bodyHeight - 110) + 'px;'">
      ...
      >
        <template slot="classificationNames" slot-scope="row">
          <span v-if="row.classificationNames && row.classificationNames.split(',').length <= 5">
            <a-tag
          ...
          <span
            v-else-if="row.classificationNames && row.classificationNames.split(',').length > 5"
          >
            <template>
              <a-tooltip>
                <template slot="title">
                  <span v-for="(item, index) in row.classificationNames.split(',')" :key="index">
                    {{ item }}
                    <br />
                  </span>
                </template>
                <span>
                  <a-tag
                    v-for="(tag, index) in row.classificationNames.split(',').splice(0, 5)"
                    color="green"
                    :key="index"
                    >{{ tag }}</a-tag
                  >
        ...
        <template slot="renderCatalog" slot-scope="text">
          <a-tooltip placement="top" v-if="renderCatalogText(text).length > 15">
            <template slot="title">
              <span style="word-break:break-all">{{ renderCatalogText(text) }}</span>
            </template>
    ...
    >
      <a-spin size="large" :spinning="sourceTableLoading">
        <div class="table-search">
          ...
          <div v-else class="eidtForm">
            <a-descriptions bordered size="small" class="des">
              ...
              <a-descriptions-item label="来源表名">
                ...
                <vue-scroll :style="'height:' + (bodyHeight - 295) + 'px;'" ref="vs">
                  <a-menu :selectedKeys="currentSelectedKey">
                    <a-menu-item
                      ...
                      :disabled="!!formData.id"
                    >
                      <a-tooltip :title="item.tableName">
                ...
                <a-layout-content class="contentTable">
                  ...
                  >
                    ...
                    <template slot="typeName" slot-scope="text, record, index">
                      <div class="editable-cell">
                        <div
                          v-if="record.editableTypeName === true && record.selectTypeName == false"
                        ...
                        >
                          <a-select
                            ...
                            style="width: 150px; text-align: center"
                            placeholder="请选择类型"
                            @select="changeTypeName(text, record, index)"
                          >
                            <a-select-option
                          ...
                          <a-icon
                            type="check"
                            class="editable-cell-icon-check"
                            @click="checkTypeName(record, index)"
                          />
                        </div>
                        <div
                          v-else-if="
                            record.editableTypeName === false && record.selectTypeName === true
                          "
                        >
                          <a-input
                            v-model="record.typeName"
        ...
        <div class="footer" :style="`margin-top:${this.createIndex ? '20px' : '90px'}`">
    ...
    >
      <a-spin size="large" :spinning="spinDetails">
        <a-row>
          <a-col :span="12">
            <div>
              <h3 style="margin-bottom: 20px">列信息详情</h3>
              <vue-scroll style="height:220px">
                <a-table
              ...
              <span v-else>无</span>
                  ...
                  <span slot="customTitle">
                    ...
                    &nbsp; &nbsp;
                    <span>
                      <a-tooltip title="可点击列名查看数据标准详情(如果该列名存在标准)">
                        <a-icon type="question-circle" />
                      </a-tooltip>
                    </span>
                    &nbsp; &nbsp;
                    <span>
                      <a-tooltip :title="columnsList.join(',')">
                        <a-icon type="form" />
                      </a-tooltip>
                    </span>
                  </span>
                  <template slot="name" slot-scope="text, record">
                    <a-button
                      type="link"
                      slot="name"
                      @click="getStandard(record)"
                      v-if="record.bindStandardId"
                      >{{ text }}</a-button
                    >
          ...
          <a-col :span="12">
            <div id="main" style="height:220px;margin-left:20px"></div>
          </a-col>
        </a-row>
        <a-row style="margin-top:50px">
            ...
            <div>
              <a-descriptions title="元数据详情" bordered :column="1" size="small">
                ...
                <a-descriptions-item label="来源类型">
                  {{ details.originTypeName }}
                </a-descriptions-item>
                <a-descriptions-item label="标签">
                  {{ details.classificationNames }}
                </a-descriptions-item>
          ...
          <a-col :span="11" style="margin-left:30px">
            <a-descriptions title="所属数据源详情" bordered :column="1">
              <a-descriptions-item label="所属数据源名称">
                {{ dataSourceDetails.name }}
              </a-descriptions-item>
              <a-descriptions-item label="所属数据源类型">
                {{
                  dataSourceDetails.sourceType == "0"
                    ? "BigData"
                    : dataSourceDetails.sourceType == "1"
                    ? "JDBC"
                    : "Kafka"
                }}
              </a-descriptions-item>
              <a-descriptions-item label="所属数据源URL">
              ...
              <a-tab-pane key="2" tab="数据标准">
                <div style="padding: 20px; color: rgba(0, 0, 0, 0.85)">
                  <p style="margin-left: 40px; margin-bottom: 20px">
                    <span>是否允许空值 : </span>
                    <span>
                      {{ standardDetails.nullable == "0" ? "不可空" : "可空" }}
                    </span>
                  </p>
                  <p style="margin-left: 40px; margin-bottom: 20px">
                    <span>是否允许重复 : </span>
                    <span>
                      {{ standardDetails.repeatable == "0" ? "不可重复" : "可重复" }}
                    </span>
                  </p>
                  <a-form-model>
                    <a-form-model-item
                      ...
                      :wrapper-col="{ span: 20 }"
                    >
                      <a-select style="width: 140px" placeholder="请选择运算符" :disabled="true">
                      ...
                      <a-input
                        ...
                        :disabled="true"
                        v-model="standardDetails.valueRange.postValue"
                      ></a-input>
                      <a-button shape="circle" :disabled="true" style="margin: 0px 5px">
                        {{
                          standardDetails.valueRange.relationType == "0"
                            ? "关"
                            : standardDetails.valueRange.relationType == "1"
                            ? "或"
                            : "且"
                        }}
                      </a-button>
                      <a-select
                        ...
                        placeholder="请选择运算符"
                        :disabled="true"
                        v-model="standardDetails.valueRange.preOperator"
                      >
                        <a-select-option value="="> = </a-select-option>
                        <a-select-option value="<"> &lt; </a-select-option>
                        <a-select-option value=">"> &gt; </a-select-option>
                      ...
                      <a-input
                        ...
                        style="width: 8.5714rem; margin-left: 5px"
                        :disabled="true"
                        v-model="standardDetails.valueRange.preValue"
                      ></a-input>
                    </a-form-model-item>
                  </a-form-model>
  ...
  data() {
    return {
      ...
      tableData: [
        ...
        //   name:"ysj_qw",
        //   aliases:"勤务",
        //   assetCatalogIds:"元数据/勤务",
        //   classificationNames:"信息,出勤",
        //   createdTime:"2021-07-16 13:12:45",
        //   updateTime:"2021-07-18 14:22:43",
        //   originTypeName:"oracle",
        //   dataSourceId:"b_oracle",
        //   typeName:"oracle"
        // },
        // {
        //   name:"ysj_gc",
        //   aliases:"过车",
        //   assetCatalogIds:"元数据/过车",
        //   classificationNames:"过车流",
        //   createdTime:"2021-07-17 09:33:12",
        //   updateTime:"2021-07-18 17:27:11",
        //   originTypeName:"mysql",
        //   dataSourceId:"a_mysql",
        //   typeName:"mysql"
        // },
      ],
      currentPage: 1,
      ...
      standardSpinning: false,
      myChart: null
    };
  },
  methods: {
    add() {
      this.changeTableNameFlag = false;
      ...
      this.resetFields();
      this.getCatalogList();
      this.$set(this.formDataParent, "assetCatalogId", []);
      ...
      //   this.formData.originTypeName="kafka"
      // }
      this.formData.originTypeName =
        this.sourceType == "0" ? "hive_table" : this.sourceType == "1" ? null : "kafka";
      this.renderColumns();
      //默认当前获取数据的为大数据类型
      this.formDataParent.dataSourceId = this.dataSourceList1[0].value;
    ...
    edit(row) {
      ...
      this.$http({
        ...
        .then(res => {
          if (res.data.success) {
            ...
            this.$set(
              this.formDataParent,
              "assetCatalogId",
              this.formData.assetCatalogIds.split(",")
            );
            ...
            if (this.formData.classificationNames == "") {
              ...
              this.$set(this.$refs.formChild.formChild, "classificationNames", []);
            } else {
              // this.$refs.formChild.formChild.classificationNames =
              //   this.formData.classificationNames.split(",");
              this.$set(
                this.$refs.formChild.formChild,
                "classificationNames",
                this.formData.classificationNames.split(",")
        ...
        .then(res => {
          //获取数据源下的表
          if (this.notEditable) {
            this.sourceTableData = this.formData.columns;
    ...
    onCloseEdit() {
      ...
      console.log(this.formData.esColumns);
      try {
    ...
    getDetails(row) {
      ...
      //   this.columnDetails = this.details.columns;
      // } else {
      //   this.columnDetails = [];
      // }
      ...
      this.$http({
        ...
        if (res.data.success) {
          this.dataSourceDetails = Object.assign(...res.data.data.results);
          console.log(this.dataSourceDetails);
        } else {
          this.$message.error("获取所属数据源详情失败", 2);
        }
        console.log(row.columns);
        let columns = row.columns.map(item => {
          return item.id;
        });
        console.log(columns);
        this.$http({
          method: "get",
          url: "/zuul/lmanage/dataEntity/columnStandard",
          params: {
            ids: columns.join(",")
          }
        }).then(res => {
          if (res.data.success) {
            this.columnDetails = res.data.data;
          }
      ...
      this.$http({
        ...
        if (this.sourceType == "2") {
          if (res.data.data.success) {
            this.sourceTableData = res.data.data.data.columns;
            try {
              this.$refs["vs-table"].scrollTo({ y: 0 }, 500);
            } catch (error) {
              console.log(error);
            ...
            if (res.data.data.data.entityDetailMap == null) {
              this.formData.rawLineageImpactPoint = null;
            } else {
              this.formData.rawLineageImpactPoint = JSON.stringify(
                res.data.data.data.entityDetailMap
              );
            }
            if (res.data.data.data.relations == null) {
              this.formData.rawLineageImpact = null;
            } else {
              this.formData.rawLineageImpact = JSON.stringify(res.data.data.data.relations);
            }
            console.log(this.formData);
            this.sourceTableLoading = false;
          } else {
            this.$message.error(res.data.data.message, 2);
            this.sourceTableLoading = false;
            this.sourceTableData = [];
          }
        } else {
        ...
        .then(res => {
          ...
          if (this.changeTableNameFlag) {
            ...
            this.$http({
              ...
              console.log(arr[0].sourceType);
              if (arr[0].sourceType == "2") {
                if (res.data.data.success) {
                  if (this.notEditable) {
                    ...
                    for (var i = 0; i < this.sourceTableData.length; i++) {
                      for (var j = 0; j < currentColumns.length; j++) {
                        if (this.sourceTableData[i].columnName == currentColumns[j].columnName) {
                          ...
                          // currentColumns[j].typeName =
                          //   this.sourceTableData[i].typeName;
                        }
                      }
                    }
                ...
                if (res.data.success) {
                  ...
                  // console.log("列数据", res);
                  //如果entityDetailMap为空 则formData中的rawLineageImpactPoint为空
                  if (arr[0].sourceType != "3") {
                    if (res.data.data.entityDetailMap == null) {
                      this.formData.rawLineageImpactPoint = null;
                    } else {
                      //反之序列化 并且给血缘赋值
                      this.formData.rawLineageImpactPoint = JSON.stringify(
                        res.data.data.entityDetailMap
                      );
                    }
                    //如果relations为空 则formData中的rawLineageImpact为空
                    if (res.data.data.relations == null) {
            ...
            if (valid) {
              if (this.notEditable) {
                ...
                if (this.formData.columns == "") {
                  ...
                  this.$refs.formChild.accurateValue.length == 0 &&
                  this.createIndex &&
                  this.$refs.formChild.blurryValue.length == 0
                ) {
                  this.$message.error("请至少选择一个模糊索引或者精确索引");
                  this.btnLoading = false;
                } else {
                  this.$refs.formChild.$children[0].validate(valid => {
                    this.btnLoading = true;
                    ...
                    if (valid) {
                      ...
                      this.$http({
                        ...
                        if (res.data.success) {
                          ...
                          // this.formData.columns = JSON.parse(this.formData.columns);
                          if (this.formData.esColumns.length > 0 && this.formData.esColumns != "") {
    ...
    //     if (
    //       this.selectedIds[this.selectedIds.length - 1] > 12 &&
    //       !this.notEditable
    //     ) {
    ...
    searchTableChange() {
      ...
      //           } else {
      //             //反之序列化 并且给血缘赋值
      //             this.formData.rawLineageImpact = JSON.stringify(
    ...
    checkTypeName(record) {
      // console.log(record.comment);
      let index = this.formData.columns.findIndex(item => {
        return item.columnName == record.columnName;
      });
      this.formData.columns[index].typeName = record.typeName;
      console.log(this.formData.columns);
      record.editableTypeName = false;
      record.selectTypeName = false;
      this.$delete(record, "editableTypeName");
      // this.$delete(record, "selected");
      console.log(this.formData.columns);
    },
    editComment(record) {
    ...
    editTypeName(record) {
      console.log(123);
      this.$set(record, "editableTypeName", true);
      this.$set(record, "selectTypeName", false);
      // record.editable = true;
      console.log(record.editableTypeName);
    },
    ...
    getStandard(row) {
      // console.log(row.bindStandardId);
      this.standardVisible = true;
      this.standardSpinning = true;
      this.detailTab = "1";
      this.eData = [];
      this.$http({
        method: "get",
        ...
        if (res.data.success) {
          ...
          this.standardDetails = res.data.data.results[0];
          this.standardDetails.valueRange = JSON.parse(res.data.data.results[0].valueRange);
          if (this.standardDetails.enumRange === null) {
            this.eData = [];
          } else {
          this.standardDetails.enumRange = JSON.parse(res.data.data.results[0].enumRange);
            this.standardDetails.enumRange.forEach(item => {
              this.eData.push({
                enumValue: item.key,
          ...
          console.log(this.standardDetails.valueRange);
        } else {
          this.$message.error("获取数据标准详情失败", 2);
        }
        this.standardSpinning = false;
      });
    },
    changeTypeName(text, record, index) {
      console.log(record);
      this.$set(record, "selectTypeName", true);
      this.$set(record, "editableTypeName", false);
    },
    initChart(row) {
      console.log(row);
      var arr = [];
      var rawLineageImpact = JSON.parse(row.rawLineageImpact);
      var rawLineageImpactPoint = JSON.parse(row.rawLineageImpactPoint);

      console.log(rawLineageImpact);
      var chartDom = document.getElementById("main");
      this.myChart = echarts.init(chartDom);
      var option;
      var arr = [];
      for (var key in rawLineageImpact[0]) {
        if (key === "fromEntityId") {
          arr.push({
            name:
              rawLineageImpactPoint["0"].attributes.name +
              "</br>" +
              rawLineageImpactPoint["0"].attributes.qualifiedName +
              "</br>" +
              rawLineageImpactPoint["0"].displayText +
              "</br>" +
              rawLineageImpactPoint["0"].status +
              "</br>" +
              rawLineageImpactPoint["0"].typeName,
            id: rawLineageImpact[0][key]
          });
        } else {
          arr.push({
            name: "名称:  " + row.name + "<br/>" + "中文名称:  " + row.aliases,
            id: rawLineageImpact[0][key]
          });
        }
      }
        ...
        series: [
          {
            ...
            data: [
              {
                ...
                y: 300,
                symbol:
                  "image://data:image/png;base64,"
              },
              {
                name: arr[1].name,
                id: arr[1].id,
                ...
                y: 300,
                symbol:
                  "image://data:image/png;base64,"
              }
              // {
              //   name: arr[1].toEntityId,
              //   x: 300,
              //   y: 300,
              // },
            ],
            ...
            links: [
              ...
              // {
              //   source: arr[0].toEntityId,
              //   target: arr[1].toEntityId,
              //   // label: {
              //   //   show: true,
              //   //   formatter:function(params){
              //   //     return `源:${params.data.source}`
              //   //   }
              //   // },
              // },
            ],
  ...
  computed: {
    ...
    filterTableNameList() {
      return this.tableNameList.filter(k => {
        return k.tableName.toUpperCase().match(this.searchTableName.toUpperCase());
      });
    },
    columnsList() {
      return this.columnDetails.map(item => {
